...
function loadState() {
    try {
        ...
        if (saved) {
            ...
            if (loaded.gameInProgress) { state = loaded; return true; }
        }
    } catch (e) {}
    return false;
}
...
function updateImpostorSelector() {
    ...
    selector.innerHTML = '';
    for (let i = 1; i <= Math.min(maxPossible, 4); i++) {
...
function generatePlayerInputs() {
    ...
    for (let i = 1; i <= state.playerCount; i++) {
        ...
        div.innerHTML = '<div class="player-number">' + i + '</div><input type="text" id="player-' + i + '" placeholder="Jogador ' + i + '" maxlength="12">';
...
function showGameScreen() {
    ...
    gamePlayers.innerHTML = state.players.map(name => '<span class="player-chip">' + name + '</span>').join('');
...
function showRoundEnd() {
    ...
    document.getElementById('overlay-container').innerHTML = '<div class="confirm-overlay"><div class="confirm-box"><h3>🎭 REVELAÇÃO</h3><p style="margin-bottom:8px;color:var(--text-dim);font-size:.8rem">' + (state.actualImpostorCount > 1 ? 'Os impostores eram:' : 'O impostor era:') + '</p><p style="font-size:1.4rem;color:var(--accent);margin:12px 0;font-family:\'Bebas Neue\',sans-serif;letter-spacing:2px">' + impostorNames.join(', ') + '</p><p style="margin:16px 0;font-size:.85rem">Palavra: <strong style="color:var(--success)">' + state.word + '</strong></p><div class="section-title">PONTUAÇÃO (OPCIONAL)</div><div class="score-controls">' + scoreControlsHTML + '</div><button class="btn btn-primary" onclick="closeOverlay();newRound();" style="margin-top:16px">PRÓXIMA RODADA</button><button class="btn btn-secondary" onclick="closeOverlay();" style="margin-top:8px">FECHAR</button></div></div>';
}
...
function resetGame() {
    closeOverlay();
    state = { playerCount: 4, maxImpostors: 1, players: [], scores: {}, currentPlayerIndex: 0, word: null, category: null, impostorIndices: [], actualImpostorCount: 0, round: 1, seenPlayers: [], gameInProgress: false };
    clearState();
    generatePlayerInputs();
